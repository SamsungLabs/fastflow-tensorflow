...
Status IteratorResource::GetNext(OpKernelContext* ctx,
  ...
  if (collect_metrics_) {
    ...
    int64_t total_bytes = GetTotalBytes(*out_tensors);
    const uint64 elapsed_lifetime = safe_sub(end_time_us, get_next_end_time_us_);
    const uint64 duration_us = safe_sub(end_time_us, start_time_us);

    metrics::RecordTFDataGetNextDuration(duration_us);
    metrics::RecordTFDataBytesFetched(total_bytes);
    mutex_lock l(mu_);
    metrics::RecordTFDataIteratorLifetime(
        safe_sub(end_time_us, get_next_end_time_us_));

    total_get_next_amount_b_ += total_bytes;
    total_iterator_life_time_us_ += elapsed_lifetime;
    total_get_next_duration_us_ += duration_us;
...
Status IteratorGetBytesOp::DoCompute(OpKernelContext* ctx) {
  tensorflow::ResourceTagger tag(kTFDataResourceTag,
                                 ctx->op_kernel().type_string());
  IteratorResource* iterator;
  TF_RETURN_IF_ERROR(LookupResource(ctx, HandleFromInput(ctx, 0), &iterator));
  ...
  Tensor* thp_handle_t = NULL;
  TF_RETURN_IF_ERROR(ctx, ctx->allocate_output(0, TensorShape({}), &thp_handle_t));
  auto output_t = thp_handle_t->flat<int64>();
  output_t(0) = iterator->GetProcessedBytes();

  return Status::OK();
}

Status IteratorGetProcessingTimeOp::DoCompute(OpKernelContext* ctx) {
  tensorflow::ResourceTagger tag(kTFDataResourceTag,
                                 ctx->op_kernel().type_string());
  IteratorResource* iterator;
  TF_RETURN_IF_ERROR(LookupResource(ctx, HandleFromInput(ctx, 0), &iterator));
  ...
  TF_RETURN_IF_ERROR(ctx, ctx->allocate_output(0, TensorShape({}), &thp_handle_t));
  auto output_t = thp_handle_t->flat<int64>();
  output_t(0) = iterator->GetProcessingTime();
...
REGISTER_KERNEL_BUILDER(Name("IteratorGetNext").Device(DEVICE_GPU).Priority(1),
                        IteratorGetNextOp);
REGISTER_KERNEL_BUILDER(Name("IteratorGetBytes").Device(DEVICE_CPU).Priority(1),
                        IteratorGetBytesOp);
REGISTER_KERNEL_BUILDER(Name("IteratorGetProcessingTime").Device(DEVICE_CPU).Priority(1),
                        IteratorGetProcessingTimeOp);
REGISTER_KERNEL_BUILDER(
