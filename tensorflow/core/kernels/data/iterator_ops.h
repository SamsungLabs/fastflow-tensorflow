...
class IteratorResource : public ResourceBase {
 public:
  ...
  int64 GetProcessedBytes() const {
    return total_get_next_amount_b_;
  }

  int64 GetProcessingTime() const {
    return total_iterator_life_time_us_;
  }
 ...
 private:
  ...
  // Records the total amount of data (in megabytes) read from the iterator
  uint64 total_get_next_amount_b_ TF_GUARDED_BY(mu_) = 0;
  // Records the total time (in seconds) to read from the iterator
  uint64 total_iterator_life_time_us_ TF_GUARDED_BY(mu_) = 0;
  uint64 total_get_next_duration_us_ TF_GUARDED_BY(mu_) = 0;
...
class IteratorGetBytesOp : public HybridAsyncOpKernel {
public:
  explicit IteratorGetBytesOp(OpKernelConstruction* ctx)
      : HybridAsyncOpKernel(ctx, "tf_data_iterator_get_bytes") {
  }
...
class IteratorGetProcessingTimeOp : public HybridAsyncOpKernel {
public:
  explicit IteratorGetProcessingTimeOp(OpKernelConstruction* ctx)
      : HybridAsyncOpKernel(ctx, "tf_data_iterator_processing_time") {
  }
