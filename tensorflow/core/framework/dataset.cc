...
Status DatasetBase::MakeSplitProviders(
  ...
  if (type_string().compare("FastflowOffloadingFetchV2") == 0 || type_string().compare("DataServiceDatasetV2") == 0) {
    // Skip making split_providers when leaf node is FastFlowOffloadingFetchV2 or DataServiceDatasetV2 for middle offloading
    return Status::OK();
  }
  std::vector<const DatasetBase*> inputs;
  Status s = InputDatasets(&inputs);
