...
REGISTER_OP("IteratorGetBytes")
    .Input("iterator: resource")
    .Output("bytes: int64")
    .SetShapeFn(shape_inference::ScalarShape);

REGISTER_OP("IteratorGetProcessingTime")
        .Input("iterator: resource")
        .Output("time: int64")
        .SetShapeFn(shape_inference::ScalarShape);
...
REGISTER_OP("OptionsDataset")
    ...
    .Attr("address_list: list(string) = []")
    .Attr("thread_list: list(int) = []")
    .Attr("target_threadpool_size: int = -1")
    .Attr("output_types: list(type) >= 1")
    .Attr("output_shapes: list(shape) >= 1")
    .Attr("metadata: string = ''")
    .SetShapeFn(shape_inference::ScalarShape);
