...
Status DataServiceDispatcherImpl::AssignTask(std::shared_ptr<const Task> task)
  ...
  if (!s.ok()) {
    if (s.error_code() == grpc::StatusCode::UNAVAILABLE ||
      ...
      // when it reconnects.
      VLOG(1) << "Worker is presumably preempted. We will assign the task to the worker " << task->worker_address;
      return Status::OK();
    }
      VLOG(1) << "Failed to submit task to worker " << task->worker_address;
