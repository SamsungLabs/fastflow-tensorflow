...
name=None))
  ...
  def with_options(self, options, address_list=[], thread_list=[],
                   target_threadpool_size=-1, name=None):
    ...
    Args:
      ...
      address_list: (Optional.) Worker address list that need
                    private_threadpool_size manually.
                    address_list length MUST same with thread_list length.
                    if worker address matched with address_list,
                    then private_threadpool_size will override with thread_list.
      thread_list: (Optional.) private_threadpool_size list
      target_threadpool_size: (Optional.) override private_threadpool_size
    ...
    return _OptionsDataset(self, options,
                           address_list=address_list, thread_list=thread_list,
                           target_threadpool_size=target_threadpool_size,
                           name=name)
...
class _OptionsDataset(UnaryUnchangedStructureDataset):
  ...
  def __init__(self, input_dataset, options,
    ...
    if name or compat.forward_compatible(2021, 9, 30):
      kwargs["metadata"] = self._metadata.SerializeToString()
    if len(address_list) != len(thread_list):
      raise ValueError(f"Address list length and Thread list length "
                       f"are different with.")
    kwargs["address_list"] = address_list
    kwargs["thread_list"] = thread_list
    kwargs["target_threadpool_size"] = target_threadpool_size
    with ops.colocate_with(input_dataset._variant_tensor):
