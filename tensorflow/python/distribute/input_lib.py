...
class DistributedIteratorBase(DistributedIteratorInterface):
  ...
  def get_bytes(self):
    ...
    for i, worker in enumerate(self._input_workers.worker_devices):
      with ops.device("/cpu:0"):
        total_bytes += self._iterators[i].get_bytes()
    return total_bytes
  ...
  def get_processing_time(self):
    ...
    for i, worker in enumerate(self._input_workers.worker_devices):
      with ops.device("/cpu:0"):
        total_time += self._iterators[i].get_processing_time()
    return total_time
...
class _SingleWorkerDatasetIteratorBase(object):
  ...
  def get_bytes(self):
    with ops.device("/cpu:0"):
        return self._iterator.get_bytes()

  def get_processing_time(self):
    with ops.device("/cpu:0"):
      return self._iterator.get_processing_time()
